...
def simulate_account(trades, stake=1000.0):
    ...
    # Capital required on hand: the most the book ever held
    ...
    # losing streak that had to be sat through without quitting.
    worst_drawdown = 0.0
    running_peak = 0.0
...
def simulate_fixed_capital(trades, capital=25000.0, stake=1000.0, seed=None,
    ...
    # exactly what the gate exists to avoid. When the gate is on and there is
...
def format_report(trades, benchmark=None, stake=1000.0, label="", reference=None):
    ...
    add(f"  So {money(account['capital_required'])} was needed on hand to take every signal.")
