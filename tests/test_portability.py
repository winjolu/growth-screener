...
class PortabilityTest(unittest.TestCase):
    ...
    def test_the_optional_data_caches_are_configurable(self):
        # Each reads an environment variable with a home-relative default,
        # so a machine without the cache skips rather than fails. Modules
        # absent from this fork are skipped rather than asserted on: the
        # rule is about the ones that exist, not about which exist.
        checked = 0
        for module, name in (("tests.test_data_agreement", "SHARADAR_CACHE"),
            ...
            checked += 1
        self.assertGreater(checked, 0, "no cache-backed module found to check")
